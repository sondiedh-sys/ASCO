...
let rec check_expr = function
  ...
  | EUnop (_, e) ->
      check_expr e
  | ECall (f, args) ->
      prerr_endline ("DEBUG: checking call to " ^ f);
      begin match lookup f with
      | Some { s_kind = KFunc; _ } -> prerr_endline ("  -> found as function")
      | Some _ -> prerr_endline ("  -> found but not a function"); scope_error "identifier is not a function" f
      | None -> prerr_endline ("  -> not found"); scope_error "call to undeclared function" f
      end;
...
let check_scope (f:file) =
  ...
  List.iter (fun fn ->
    prerr_endline ("DEBUG: Processing function " ^ fn.f_name);
